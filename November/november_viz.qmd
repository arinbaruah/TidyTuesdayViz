...
pl_midnotes <- ggplot(
  ...
  labs(title = "Most popular middle notes of highly rated perfumes") +
...
pl_basenotes <- ggplot(
  ...
  geom_text_wordcloud_area(
    ...
    labs(title = "Most popular base notes of highly rated perfumes") +
  scale_size_area(max_size = 20) +
...
pl_topnotes <- ggplot(
  ...
  geom_text_wordcloud_area(
    ...
    rm_outside = TRUE
      ...
      labs(title = "Most popular top notes of highly rated perfumes") +
  scale_size_area(max_size = 20) +
  scale_color_gradient(low = "#8E44AD", high = "#3498db")
```
...
subtitle_text <- "<span style='color:#DDAA0C'><strong>Hermès</span></strong> fragrances lead the pack with an impressive average rating of 8.8, securing their place as the highest-rated <br> brand.Following closely are <span style='color:#236A82'><strong>Roja Parfums</span></strong> with a strong 8.4, <span style='color:#D0C81A'><strong>Jean Patou</span></strong> at 8.3, and <span style='color:#D48FDC'><strong>Chanel and Tous</span></strong>, tied at 8.2, <br> rounding out the top five highest-rated perfume brands."
...
pl_final <- (pl_perfume)/(pl_basenotes + pl_midnotes + pl_topnotes) +
             plot_layout(heights = c(4,2))

ggsave("Fragrance_infographic.jpeg",plot = pl_final,width =9, height = 6)
```
