...
# plot colors
bkg_col      <- colorspace::lighten("#93CCEA", 0.05)    
title_col    <- "#3d3d3d"           
subtitle_col <- "#3d3d3d"     
caption_col  <- "#72647D"   
text_col     <- colorspace::darken("gray40" , 0.2)  
...
# Create the plot
ggplot(data = ewf_standings, aes(x = goals_for, y = goals_against, text = team_name)) +
  geom_point(aes(color = Status), size = 1) + geom_point(data = filter(ewf_standings, Status == "Relegations"),
                                                         aes(color = Status), size = 4) +
  geom_point(data = filter(ewf_standings, Status == "Champions"),
                                                         aes(color = Status),size = 4) +
  geom_label_repel(data = filter(ewf_standings, Status == "Relegations"),
                   aes(label = team_name), size =2, nudge_x = -7, nudge_y = 18) +
    geom_label_repel(data = filter(ewf_standings, Status == "Champions"),
                   aes(label = team_name), size = 2, nudge_x = 7, nudge_y = 9) +
