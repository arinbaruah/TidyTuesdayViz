...
```
```{r}
ggplot(data = summer_movies,aes(x = average_rating)) + geom_histogram()
```
...
ggplot(data = summer_movies %>% filter(num_votes < 200000),aes(x = runtime_minutes,y = num_votes,colour = is_top)) + geom_point()
```
...
ggplot(summer_movie_cats, aes(id = genres,area = Total,fill = Total)) +
  geom_circlepack() +
  geom_circlepack_text(aes(label = genres, family = "title", fontface = "bold",color = "white")) +
  geom_circlepack_text(aes(y = after_stat(y - 2), label = str_wrap(Total, 8), 
