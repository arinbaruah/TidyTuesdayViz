...
lineplot <- ggplot(data = summer_movies_number,aes(x = year,y =rollapply(summer_movies_number$Total, width = 5, FUN = mean, align = "right", fill = NA))) + geom_line() 
```

```{r}
library(patchwork)
summer_movie_cats <- summer_movie_genres %>% group_by(genres) %>% summarise(Total = n())

cc <- scales::seq_gradient_pal(pal)(seq(0, 1, length.out = 18))

circles <- ggplot(summer_movie_cats, aes(id = genres,area = Total,fill = Total)) +
  geom_circlepack() +
