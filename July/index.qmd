...
# Create a new column to distinguish between general points and highlighted points
ewf_standings$highlight <- ifelse(ewf_standings$position == 1, "Top Position", "Other Teams")
...
ggplot(data = ewf_standings, aes(x = goals_for, y = goals_against, text = team_name)) +
  ...
  geom_label_repel(data = filter(ewf_standings, position == 1),
                   aes(label = team_name), size = 3, nudge_x = 7, nudge_y = 6) +
  scale_color_manual(values = c("Top Position" = "gold", "Other Teams" = "gray"),
                     name = "Team Position") +
  theme_minimal()
```
