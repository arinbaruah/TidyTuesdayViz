...
songs <- songs %>% mutate(result_cleaned = if_else(result %in% negative_result,"Negative","Postive"))


multiple_apps <- songs %>% group_by(artist) %>% summarise(Total = n()) %>% filter(Total >= 10)

songs_grouped <- songs %>% filter(artist %in% multiple_apps$artist) %>% 
  group_by(artist,result_cleaned) %>% summarise(Total = n()) 
...
ggplot(data = songs_grouped,aes(x = Total, y = artist, fill = result_cleaned)) + 
  geom_col(position  = "fill",color = "black") + theme_minimal() + geom_vline (xintercept  = 0.5,linetype = 2) +
  labs(fill = "Outcome",
         x  = "Proportion of songs",
         y  =  " Artist's song performed",
       caption = caption_text) +
  scale_x_continuous(expand = c(0, 0), labels = scales::percent) +
   theme(legend.position = "top",
    legend.title.position = "top",
