...
# plot colors
bkg_col      <- colorspace::lighten("#CAE7D3", 0.05)    
title_col    <- "white"           
subtitle_col <- "white"     
caption_col  <- "#1690FE"   
text_col     <- colorspace::darken("white" , 0.2)  
...
ggplot(data = summer_movies,aes(x = factor(year),y = average_rating)) + geom_boxplot()
```


```{r}
summer_movies_number <- summer_movies %>% group_by(year) %>% summarise(Total = n())

ggplot(data = summer_movies_number,aes(x = year,y = Total)) + geom_line() + geom_smooth()
```
```{r}
...
```{r}
summer_movies %>% group_by(genres) %>% summarise(Total = n()) %>% arrange(-Total)

ggplot(data = summer_movies,aes(y = factor(year),x = runtime_minutes)) + geom_boxplot()

ggplot(data = summer_movies,aes(x = average_rating,y = runtime_minutes)) + geom_hex()
...
circles <- ggplot(summer_movie_cats, aes(id = genres,area = Total,fill = top_genre)) +
  geom_circlepack() +
  geom_circlepack_text(aes(label = genres, family = "title", fontface = "bold",color = "white")) +
  geom_circlepack_text(aes(y = after_stat(y - 2), label = str_wrap(Total, 4), 
  ...
  theme(
    legend.position = "none",
    plot.background = element_rect(fill = "#CAE7D3", color = NA),
...
(lineplot + hist) / circles +
  plot_layout(nrow=2, heights = c(4,10)) &
