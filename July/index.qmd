...
drob_funs_grouped <- drob_funs_grouped %>%s
...
# plot colors
bkg_col      <- colorspace::lighten("#1656AD", 0.05)    
title_col    <- "white"           
subtitle_col <- "white"     
caption_col  <- "#1690FE"   
text_col     <- colorspace::darken("white" , 0.2)  

# fonts

font_add('fa6-brands','fontawesome/otfs/Font Awesome 6 Brands-Regular-400.otf') 
font_add_google("Playfair Display", regular.wt = 400, family = "title")                 
font_add_google("Urbanist", regular.wt = 400, family = "subtitle")  
font_add_google("Quattrocento Sans", regular.wt = 400, family = "text")        
font_add_google("Merriweather", regular.wt = 400,family = "caption")
showtext_auto(enable = TRUE)  
...
caption_text <- str_glue("{li} Arindam Baruah | {X_icon} @wizsights | {gh} arinbaruah | Source: TidyTuesday |#rstudio #ggcirclepack #ggplot2")
...
negative_result <- c("Disqualified","Bottom","Not Chosen","Bottom three (Males)","Bottom 2",
                     "Bottom 3","Bottom three","Bottom Three","Bottom three (Female)",
                     "Bottom three (Males)","Bottom two","Bottom Two","Bottom two (Female)",
                     "Bottom two (Males)","Eliminated","Eliminated (6th)","Top 3/Eliminated")
winner <- c("Winner")

songs <- songs %>% mutate(result_cleaned = case_when(result %in% negative_result ~ "Eliminated",
                                                      result %in% winner ~ "Winner",
                                                      .default = "Progressed"))



songs <- songs %>% group_by(artist) %>% mutate(Total = n()) %>% filter(Total >= 10)

songs$artist_num <- paste0(
    songs$artist," <span style='font-size:18pt; color:gold'>**(",songs$Total,"**)</span>"
)


songs_grouped <- songs %>% filter(artist %in% multiple_apps$artist) %>% 
  group_by(artist_num,result_cleaned) %>% summarise(Total = n()) 

songs_grouped <- songs_grouped %>%
  group_by(artist_num) %>%
  mutate(Proportion = Total / sum(Total)) %>%
  ungroup() %>%
  arrange(-Proportion) %>%
  mutate(artist = factor(artist_num, levels = rev(unique(artist_num))))
```

```{r}
library("png")
library(cowplot)
logo <- readPNG("AmericanIdol.png")

title_text <- "American Idol Hits: Measuring the Success of the highest played Artists"


subtitle_text <- "Some artists are particularly popular choices for contestants on the American Idol stage. But how does the success rate vary for each artist's songs chosen <br> by the contestants? By studying those artists whose songs have appeared on the show at least 10 times or more (N), we uncover some fascinating statistics.

The top three most popular artists, whose songs were performed the most, are <span style='font-size:30pt; color:gold'><strong>Stevie Wonder, The Beatles, and Elton John.</span></strong>

Diving deeper into the success rate, <span style='font-size:30pt; color:gold'><strong>Dionne Warwick's</span></strong> songs emerge as the ones with the highest success rate, with several performances even by eventual <br> contest winners! This is closely followed by Aerosmith. Additionally, <span style='font-size:30pt; color:gold'><strong>Martina McBride's</span></strong> songs have produced the highest proportion of winners on this list, <br> making her a notable mention."

custom_colors <- c("Progressed" = "steelblue", "Eliminated" = "darkred", "Winner" = "gold")

p <- ggplot(data = songs_grouped,aes(x = Total, y = artist, fill = result_cleaned)) + 
  geom_col(position  = "fill",color = "black") + geom_vline (xintercept  = 0.5,linetype = 2) +
  labs(fill = "Outcome",
         x  = "Proportion of songs",
         y  =  " Artist's song performed (N)",
         title = title_text,
        subtitle = subtitle_text,
       caption = caption_text) +
  scale_x_continuous(expand = c(0, 0), labels = scales::percent) +
...
ggsave("American_Idol_success.png",p,dpi = 300,height = 6, width = 10)
...
# plot colors
bkg_col      <- colorspace::lighten("#f2b36c", 0.05)    
title_col    <- "#008080"           
subtitle_col <- "#2F4F4F"     
caption_col  <- "#008080"   
text_col     <- colorspace::darken("black" , 0.2)  
...
theme_update(
  ...
  axis.text             = element_text(size = 10, color = text_col, family = 'text',face = "bold"),
  panel.grid.minor.y    = element_blank(),
  panel.grid.major.y    = element_line(linetype = "dotted", linewidth = 0.1, color = 'black'),
  panel.grid.minor.x    = element_blank(),
  panel.grid.major.x    = element_blank(),
  axis.line.x           = element_line(color = "#d7d7d8", linewidth = .2),
)
...
```{r}
hist <- ggplot(data = summer_movies,aes(x = average_rating)) + geom_histogram() + 
  geom_vline(xintercept = mean(summer_movies$average_rating),
             color = "darkred",linetype = 3,linewidth = 2) +
  annotate(geom = "text",
           x = 4,
           y = 50,
           label = paste0("Average rating:",round(mean(summer_movies$average_rating),1)),
           size = 7,
           fontface = "bold",
           family = "caption") +
...
summer_movies_number <- summer_movies_number %>% mutate(yoy = 100*(Total - lag(Total))/(Total))

summer_movies_number <- summer_movies_number %>%
  mutate(roll_mean = rollapply(Total, width = 5, FUN = mean, align = "right", fill = NA))


lineplot <- ggplot(data = summer_movies_number,aes(x = year,y = roll_mean)) + geom_line() +
  geom_point(
    data = filter(summer_movies_number,year == 1995),
    size = 3,
    color = "darkred"
  ) +
    geom_segment(aes(x = 1995,
                   y = 8,
                   xend = 1990,
                   yend = 18),
               size = 1) +
  annotate(geom = "text",
           ...
           y = 20,
           size = 6,
           label = 'Exponential rise in "summer movies" after 1995',
           fontface = "bold",
           family = "text") +
  labs(x = "Year",
       y = "Movies  released") +
...
title_text <- "The rising growth of Summer movies over the years"

subtitle_text <- "The number of summer movies has been rising, with a notable increase observed after 1995. This trend indicates the growing <br> popularity of these films among movie enthusiasts and the advancements in technology that have facilitated the release of <br> multiple movies each year. <br> <br>
Summer movies tend to be more light-hearted, with the most common genres being <span style='font-size:25pt; color:darkred'><strong>Drama,</strong></span> followed by<span style='font-size:25pt; color:darkred'><strong> Comedy</strong></span>and <br> <span style='font-size:25pt; color:darkred'><strong>Romance </strong> </span>.
<br> <br>
On average, the rating of these movies is approximately<span style='font-size:25pt; color:darkred'><strong> 6.3 </strong></span>, with a <span style='font-size:25pt; color:darkred'><strong>left-skewed distribution</strong></span>, suggesting that while some <br> summer movies are highly rated, a significant number receive poor ratings."

summer_movie_cats <- summer_movie_genres %>% group_by(genres) %>% summarise(Total = n()) %>% mutate(top_genre = if_else(Total > 100,"Yes","No"))

cc <- c("Yes" = "#5C62D6", "No" = "gray" )

circles <- ggplot(summer_movie_cats, aes(id = genres,area = Total,fill = top_genre)) +
  geom_circlepack() +
  geom_circlepack_text(aes(label = genres, family = "title", fontface = "bold",color = "#f1fffe")) +
  geom_circlepack_text(aes(y = after_stat(y - 1), label = str_wrap(Total, 6), 
                           family = "text",color = "white",size = 800, 
  ...
  theme(
    legend.position = "none",
    plot.background = element_rect(fill = "#f2b36c", color = NA),
...
p <- circles/(lineplot + hist) +
  plot_layout(nrow=2, heights = c(10,3)) &
  plot_annotation(
    theme = theme(plot.background = element_rect(color = "#f2b36c", size = 2, fill = "#f2b36c"),
                  strip.background = element_rect(color = "#f2b36c"))
  )

ggsave("Summer_movies.png",p,dpi = 300,height = 8, width = 8)
