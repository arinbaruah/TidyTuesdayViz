...
top_langs <- lgbtq_movies %>% group_by(original_language) %>% summarise(Total = n()) %>% filter(Total > 100)
...
ggplot(data = lgbtq_movies,aes(x = vote_average)) + geom_density(fill = "red") + facet_wrap(~original_language, 
